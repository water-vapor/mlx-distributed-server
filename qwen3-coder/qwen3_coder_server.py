...
def process_message_content(messages):
    ...
    for message in messages:
        ...
        # Process tool_calls to ensure arguments are dicts for Qwen chat template
                ...
                if "function" in tool_call and "arguments" in tool_call["function"]:
                    ...
                    if isinstance(args, str):
                        try:
                            ...
                            logging.warning(f"Could not parse tool call arguments: {args}")
...
class APIHandler(BaseHTTPRequestHandler):
    def __init__(
        ...
        **kwargs,
        ...
        self.tools = None  # Will be set when processing requests
    ...
    def generate_response(
        ...
        def parse_function(tool_text):
            ...
            # Check if this is a Qwen model that outputs XML format
            if hasattr(self.tokenizer, 'name_or_path') and 'qwen' in self.tokenizer.name_or_path.lower():
                try:
                    from qwen3_xml_parser import Qwen3XMLToolParser
                    parser = Qwen3XMLToolParser()
                    return parser.parse_tool_call(tool_text.strip(), self.tools)
                except ImportError:
                    logging.warning("Could not import Qwen3XMLToolParser")
            
            # Default JSON parser for other models
            try:
        ...
        if self.object_type.startswith("chat.completion"):
            ...
            choice[key_name] = {
                "role": "assistant",
                "content": text,
                "tool_calls": [parsed for tool_text in tool_calls if (parsed := parse_function(tool_text)) is not None],
            }
    ...
    def handle_chat_completions(self) -> List[int]:
        ...
        if self.tokenizer.chat_template:
            ...
            self.tools = tools  # Store for later use in parse_function
            ...
            self.tools = body.get("tools") or None  # Store for later use
